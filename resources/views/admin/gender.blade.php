                    ...
                    <div class="table-title">
                        ...
                        <div class="row">
                            <div class="col-lg-3 float-left">
                                <form action="{{route('gender.import')}}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <a class="btn btn-warning" href="{{route('gender.export')}}" >Export User Data</a>
                                </form>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#sample_form').on('submit', function(event){
                ...
                if($('#action').val() == 'Add')
                {
                    action_url = "{{ route('gender.store') }}";
                }

                if($('#action').val() == 'Edit')
                {
                    action_url = "{{ route('gender.update') }}";
                }
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_result').html('');
                $.ajax({
                    url :"/admin/gender/"+id+"/edit",
                    dataType:"json",
                    success:function(data)
                    {
                        $('#object').val(data.result.object);
                        $('#hidden_id').val(id);
            ...
            $('#ok_button').click(function(){
                $.ajax({
                    url:"/admin/gender/destroy/"+user_id,
                    beforeSend:function(){
